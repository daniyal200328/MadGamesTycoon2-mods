...
public class Menu_ArchivEngines : MonoBehaviour
{
	...
	private void SetData(bool archiv_)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if ((bool)array[i])
			{
				...
				if ((bool)component && component.archiv_engine == archiv_ && CheckData(component) && !Exists(uiObjects[0], component.myID))
				{
					Item_ArchivEngine component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_ArchivEngine>();
	...
	{
		if ((bool)script_ && script_.myID != 0 && ((script_.isUnlocked && script_.gekauft) || (script_.ownerID == mS_.myID && script_.devPointsStart <= 0f) || (script_.ownerID == mS_.myID && script_.updating)))
		{
			return true;
		}
		return false;
