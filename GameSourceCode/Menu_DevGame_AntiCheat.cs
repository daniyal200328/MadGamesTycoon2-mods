...
public class Menu_DevGame_AntiCheat : MonoBehaviour
{
	...
	private void Init()
	{
		...
		{
			if (menuChangeCopyProtect_.g_GameAntiCheat != -1)
			{
				uiObjects[4].GetComponent<Button>().interactable = true;
			}
			else
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && component.isUnlocked && component.inBesitz && (component.effekt > 0f || !isOn) && !Exists(uiObjects[0], component.myID))
			{
				...
				if (devGame_.g_GameAntiCheat == component.myID)
				{
	...
	public void BUTTON_AntiCheatEntfernen()
	{
		if (guiMain_.uiObjects[56].activeSelf)
		{
			devGame_.SetAntiCheat(-1);
		}
		if (guiMain_.uiObjects[365].activeSelf)
		{
			menuChangeCopyProtect_.SetAntiCheat(-1);
		}
		BUTTON_Close();
	}
}
