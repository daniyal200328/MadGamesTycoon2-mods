...
public class Menu_FreewareSelect : MonoBehaviour
{
	...
	private bool Exists(GameObject parent_, int id_)
	{
		...
		{
			if (parent_.transform.GetChild(i).gameObject.activeSelf && parent_.transform.GetChild(i).GetComponent<Item_FreewareSelect>().game_.myID == id_)
			{
				return true;
			}
		}
	...
	private void SetData()
	{
		...
		{
			if ((bool)games_.arrayGamesScripts[i] && (!isOn || (isOn && !games_.arrayGamesScripts[i].GetIpIsInArchiv())) && ((!games_.arrayGamesScripts[i].freeware && TAB == 0) || (games_.arrayGamesScripts[i].freeware && TAB == 1)) && CheckGameData(games_.arrayGamesScripts[i]))
			{
				string nameSimple = games_.arrayGamesScripts[i].GetNameSimple();
				searchStringA = searchStringA.ToLower();
				nameSimple = nameSimple.ToLower();
				if (uiObjects[6].GetComponent<InputField>().text.Length <= 0 || nameSimple.Contains(searchStringA))
				{
					Item_FreewareSelect component = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_FreewareSelect>();
	...
	{
		if ((bool)script_ && script_.ownerID == mS_.myID && !script_.inDevelopment && !script_.isOnMarket && script_.gameTyp == 0 && !script_.schublade && !script_.typ_goty && !script_.typ_budget && (script_.typ_standard || script_.typ_nachfolger || script_.typ_remaster || script_.typ_spinoff) && !script_.handy && !script_.arcade && !script_.typ_bundle && !script_.typ_bundleAddon && !script_.pubOffer)
		{
			return true;
		}
		return false;
	...
	public void DROPDOWN_Sort()
	{
		...
		for (int i = 0; i < childCount; i++)
		{
			...
			Item_FreewareSelect component = gameObject.GetComponent<Item_FreewareSelect>();
			switch (value)
			{
			case 0:
			...
			case 4:
				...
				break;
			case 5:
				gameObject.name = component.game_.GetFreewareFans().ToString();
				break;
			case 6:
