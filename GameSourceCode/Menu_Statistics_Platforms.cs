...
public class Menu_Statistics_Platforms : MonoBehaviour
{
	...
	private bool Exists(GameObject parent_, int id_)
	{
		...
		{
			if (parent_.transform.GetChild(i).gameObject.activeSelf && parent_.transform.GetChild(i).GetComponent<Item_Stats_Platform>().pS_.myID == id_)
			{
				return true;
			}
		}
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && ((component.isUnlocked && TAB == 0) || (component.IsAngekuendigt() && TAB == 1)))
			{
				string text = component.GetName();
				searchStringA = searchStringA.ToLower();
				text = text.ToLower();
				if ((uiObjects[7].GetComponent<InputField>().text.Length <= 0 || text.Contains(searchStringA)) && CorrectPlatformFilter(component) && !Exists(uiObjects[0], component.myID))
				{
					Item_Stats_Platform component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_Stats_Platform>();
