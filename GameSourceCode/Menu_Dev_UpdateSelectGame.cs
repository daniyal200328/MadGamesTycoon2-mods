...
public class Menu_Dev_UpdateSelectGame : MonoBehaviour
{
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			bool flag = true;
			if (component.points_bugs <= 0f && uiObjects[9].GetComponent<Toggle>().isOn)
			{
				flag = false;
			}
			if (component.portID != -1 && uiObjects[7].GetComponent<Toggle>().isOn)
			{
				flag = false;
			}
			if ((component.typ_addon || component.typ_addonStandalone || component.typ_mmoaddon) && uiObjects[8].GetComponent<Toggle>().isOn)
			{
				flag = false;
			}
			if (flag)
			{
				...
				if ((uiObjects[6].GetComponent<InputField>().text.Length <= 0 || nameSimple.Contains(searchStringA)) && art == 0 && !Exists(uiObjects[0], component.myID))
				{
					Item_DevGame_GameUpdate component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_DevGame_GameUpdate>();
	...
	{
		if ((bool)script_ && script_.ownerID == mS_.myID && script_.developerID == mS_.myID && !script_.inDevelopment && script_.isOnMarket && !script_.typ_bundle && !script_.typ_budget && !script_.typ_bundleAddon && !script_.typ_goty && !script_.GetIpIsInArchiv())
		{
			return true;
		}
		return false;
		...
		for (int i = 0; i < childCount; i++)
		{
			...
			if ((bool)gameObject)
			{
				...
				case 4:
					...
					break;
				case 5:
					gameObject.name = component.game_.amountUpdates.ToString();
					break;
				case 6:
					gameObject.name = component.game_.points_bugs.ToString();
					break;
				}
			}
		}
