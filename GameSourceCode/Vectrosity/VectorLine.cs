...
public class VectorLine
{
	...
	private void DrawEndCap(bool draw3D)
	{
		if (m_capType <= EndCap.Mirror)
		{
			...
			if (!draw3D)
			{
				...
				m_lineVertices[m_vertexCount] = m_lineVertices[num] + vector + vector2;
				m_lineVertices[m_vertexCount + 3] = m_lineVertices[num + 3] + vector + vector2;
			...
			else
			{
				...
				m_lineVertices[m_vertexCount] = cam3D.ScreenToWorldPoint(m_screenPoints[num] + vector3 + vector4);
				m_lineVertices[m_vertexCount + 3] = cam3D.ScreenToWorldPoint(m_screenPoints[num + 3] + vector3 + vector4);
				m_lineVertices[num] = cam3D.ScreenToWorldPoint(m_screenPoints[num] + vector4);
				m_lineVertices[num + 3] = cam3D.ScreenToWorldPoint(m_screenPoints[num + 3] + vector4);
			}
		...
		if (m_capType >= EndCap.Both)
		{
			...
			if (!draw3D)
			{
				Vector3 vector5 = (m_lineVertices[num - 2] - m_lineVertices[num - 1]).normalized * m_lineWidths[num4] * 2f * capDictionary[m_endCap].ratio2;
				Vector3 vector6 = vector5 * capDictionary[m_endCap].offset2;
				m_lineVertices[m_vertexCount + 6] = m_lineVertices[num - 2] + vector5 + vector6;
				m_lineVertices[m_vertexCount + 5] = m_lineVertices[num - 3] + vector5 + vector6;
				m_lineVertices[num - 3] += vector6;
				m_lineVertices[num - 2] += vector6;
			}
			else
			{
				Vector3 vector7 = (m_screenPoints[num - 2] - m_screenPoints[num - 1]).normalized * m_lineWidths[num4] * 2f * capDictionary[m_endCap].ratio2;
				Vector3 vector8 = vector7 * capDictionary[m_endCap].offset2;
				m_lineVertices[m_vertexCount + 6] = cam3D.ScreenToWorldPoint(m_screenPoints[num - 2] + vector7 + vector8);
				m_lineVertices[m_vertexCount + 5] = cam3D.ScreenToWorldPoint(m_screenPoints[num - 3] + vector7 + vector8);
				m_lineVertices[num - 3] = cam3D.ScreenToWorldPoint(m_screenPoints[num - 3] + vector8);
				m_lineVertices[num - 2] = cam3D.ScreenToWorldPoint(m_screenPoints[num - 2] + vector8);
			}
	...
	private void SetupCanvasState(CanvasState wantedState)
	{
		if (wantedState == CanvasState.OnCanvas)
		{
			...
			if (flag)
			{
				...
				m_go.transform.SetParent(m_canvas.transform, worldPositionStays: true);
			}
	...
	private void DrawPoints()
	{
		...
		if (m_is2D)
		{
			...
			for (int i = start; i <= end; i++)
			{
				if (flag)
				{
					vector4 = thisMatrix.MultiplyPoint3x4(m_points2[i]);
				}
				else
				{
					vector4.x = m_points2[i].x;
					vector4.y = m_points2[i].y;
				}
				if (m_viewportDraw)
				{
					vector4.x *= vector.x;
		...
		else
		{
			for (int j = start; j <= end; j++)
			{
				Vector3 vector4 = (flag ? cam3D.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(m_points3[j])) : cam3D.WorldToScreenPoint(m_points3[j]));
				if (vector4.z < 0f)
				{
					SkipQuad(ref idx, ref widthIdx, ref widthIdxAdd);
					continue;
	...
	private void DrawPoints3D()
	{
		...
		for (int i = start; i <= end; i++)
		{
			vector = (flag ? cam3D.WorldToScreenPoint(thisMatrix.MultiplyPoint3x4(m_points3[i])) : cam3D.WorldToScreenPoint(m_points3[i]));
			if (vector.z < 0f)
			{
				SkipQuad(ref idx, ref widthIdx, ref widthIdxAdd);
				continue;
			...
			vector2.x = (vector2.y = (vector3.y = m_lineWidths[widthIdx]));
			vector3.x = 0f - m_lineWidths[widthIdx];
			m_lineVertices[idx] = cam3D.ScreenToWorldPoint(vector + vector3);
			m_lineVertices[idx + 3] = cam3D.ScreenToWorldPoint(vector - vector2);
			m_lineVertices[idx + 1] = cam3D.ScreenToWorldPoint(vector + vector2);
			m_lineVertices[idx + 2] = cam3D.ScreenToWorldPoint(vector - vector3);
			idx += 4;
			widthIdx += widthIdxAdd;
		}
		CheckNormals();
	...
	private void SetIntersectionPoint(int p1, int p2, int p3, int p4)
	{
		...
		if (num > -0.005f && num < 0.005f)
		{
			...
			{
				m_lineVertices[p2] = (vector2 + vector3) * 0.5f;
	...
	private void SetIntersectionPoint3D(int p1, int p2, int p3, int p4)
	{
		...
		if (num > -0.005f && num < 0.005f)
		{
			if (Mathf.Abs(vector2.x - vector3.x) < 0.005f && Mathf.Abs(vector2.y - vector3.y) < 0.005f)
			{
				m_lineVertices[p2] = cam3D.ScreenToWorldPoint((vector2 + vector3) * 0.5f);
				m_lineVertices[p3] = m_lineVertices[p2];
			}
			return;
		}
		...
		if (!((vector5 - vector2).sqrMagnitude > m_maxWeldDistance))
		{
			m_lineVertices[p2] = cam3D.ScreenToWorldPoint(vector5);
			m_lineVertices[p3] = m_lineVertices[p2];
		}
	}
	...
	private void SetTextureScale()
	{
		...
		Matrix4x4 matrix4x = (flag ? m_drawTransform.localToWorldMatrix : Matrix4x4.identity);
	...
	private void SetCollider(bool convertToWorldSpace)
	{
		...
		Vector3 v = new Vector3(0f, 0f, 0f - cam3D.transform.position.z);
		int num = drawStart;
		int num2 = drawEnd;
		...
		if (m_lineType == LineType.Continuous)
		{
			EdgeCollider2D edgeCollider2D = m_go.GetComponent(typeof(EdgeCollider2D)) as EdgeCollider2D;
		...
		PolygonCollider2D polygonCollider2D = m_go.GetComponent(typeof(PolygonCollider2D)) as PolygonCollider2D;
		...
		polygonCollider2D.pathCount = num5;
		int num6 = (num2 + 1) / 2 * 4;
		int pIdx = 0;
		if (convertToWorldSpace)
		{
			...
			{
				num3 = m_vertexCount;
				SetPathWorldVerticesDiscrete(ref num3, ref v, ref pIdx, path, polygonCollider2D);
			}
		...
		else
		{
			...
			{
				num3 = m_vertexCount;
				SetPathVerticesDiscrete(ref num3, ref pIdx, path, polygonCollider2D);
			}
			for (num3 = num / 2 * 4; num3 < num6; num3 += 4)
			{
				SetPathVerticesDiscrete(ref num3, ref pIdx, path, polygonCollider2D);
	...
	public Vector2 GetPoint(float distance, out int index)
	{
		...
		Vector2 vector = ((m_lineType == LineType.Discrete) ? Vector2.Lerp(m_points2[(index - 1) * 2], m_points2[(index - 1) * 2 + 1], Mathf.InverseLerp(m_distances[index - 1], m_distances[index], distance)) : Vector2.Lerp(m_points2[index - 1], m_points2[index], Mathf.InverseLerp(m_distances[index - 1], m_distances[index], distance)));
		if ((bool)m_drawTransform)
		{
			vector = m_drawTransform.localToWorldMatrix.MultiplyPoint3x4(vector);
		}
		index--;
		return vector;
	}
	...
	public Vector3 GetPoint3D(float distance, out int index)
	{
		...
		SetDistanceIndex(out index, distance);
		Vector3 vector = ((m_lineType == LineType.Discrete) ? Vector3.Lerp(m_points3[(index - 1) * 2], m_points3[(index - 1) * 2 + 1], Mathf.InverseLerp(m_distances[index - 1], m_distances[index], distance)) : Vector3.Lerp(m_points3[index - 1], m_points3[index], Mathf.InverseLerp(m_distances[index - 1], m_distances[index], distance)));
		if ((bool)m_drawTransform)
		{
			vector = m_drawTransform.localToWorldMatrix.MultiplyPoint3x4(vector);
		}
	...
	public static void SetEndCap(string name, EndCap capType, float offsetFront, float offsetBack, float scaleFront, float scaleBack, params Texture2D[] textures)
	{
		...
		texture2D4.Apply(flag, makeNoLongerReadable: true);
		...
		if (m_lineType == LineType.Points)
		{
			...
			if (m_is2D)
			{
				for (int i = m_drawStart; i <= num3; i++)
				{
					...
					Vector2 vector2 = (flag ? ((Vector2)matrix4x.MultiplyPoint3x4(m_points2[i])) : m_points2[i]);
					if (m_viewportDraw)
					{
						vector2.x *= vector.x;
						vector2.y *= vector.y;
			...
			for (int j = m_drawStart; j <= num3; j++)
			{
				...
				Vector2 vector2 = (flag ? cam.WorldToScreenPoint(matrix4x.MultiplyPoint3x4(m_points3[j])) : cam.WorldToScreenPoint(m_points3[j]));
	...
	private void MakeEllipse(Vector3 origin, Vector3 upVector, float xRadius, float yRadius, float startDegrees, float endDegrees, int segments, float pointRotation, int index)
	{
		...
		else
		{
			...
			{
				num = 360f;
				num2 = (0f - pointRotation) * (MathF.PI / 180f);
			}
			else
			{
				num = ((endDegrees > startDegrees) ? (endDegrees - startDegrees) : (360f - startDegrees + endDegrees));
				num2 = startDegrees * (MathF.PI / 180f);
			}
			float num3 = num / (float)segments * (MathF.PI / 180f);
			if (m_lineType != LineType.Discrete)
			{
				if (startDegrees != endDegrees)
				{
	...
	private static Vector2 GetBezierPoint(ref Vector2 anchor1, ref Vector2 control1, ref Vector2 anchor2, ref Vector2 control2, float t)
	{
		...
		return new Vector2(y: (anchor2.y - anchor1.y - num4 - num5) * (t * t * t) + num5 * (t * t) + num4 * t + anchor1.y, x: num3 * (t * t * t) + num2 * (t * t) + num * t + anchor1.x);
	}
	...
	private static Vector3 GetBezierPoint3D(ref Vector3 anchor1, ref Vector3 control1, ref Vector3 anchor2, ref Vector3 control2, float t)
	{
		...
		return new Vector3(z: (anchor2.z - anchor1.z - num7 - num8) * (t * t * t) + num8 * (t * t) + num7 * t + anchor1.z, x: num3 * (t * t * t) + num2 * (t * t) + num * t + anchor1.x, y: num6 * (t * t * t) + num5 * (t * t) + num4 * t + anchor1.y);
	}
