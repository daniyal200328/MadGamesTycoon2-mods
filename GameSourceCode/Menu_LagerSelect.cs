...
public class Menu_LagerSelect : MonoBehaviour
{
	...
	public void Init(roomScript room_)
	{
		...
		uiObjects[4].GetComponent<Toggle>().isOn = mS_.sellLagerbestandAutomatic;
		SetData();
	}
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if ((bool)array[i])
			{
				...
				{
					Item_Restbestand component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_Restbestand>();
	...
	{
		if ((bool)script_ && script_.publisherID == mS_.myID && !script_.inDevelopment && script_.GetLagerbestand() > 0)
		{
			return true;
		}
		return false;
	...
	public void BUTTON_AllesVerkaufen()
	{
		...
		guiMain_.uiObjects[226].GetComponent<Menu_W_Restbestand>().Init(null);
	}

	public void TOGGLE_Automatic()
	{
		mS_.sellLagerbestandAutomatic = uiObjects[4].GetComponent<Toggle>().isOn;
	}
}
