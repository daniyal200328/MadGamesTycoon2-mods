...
public class Menu_Stats_TochterfirmaIP : MonoBehaviour
{
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if ((bool)array[i])
			{
				...
				{
					Item_TochterfirmaIP component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_TochterfirmaIP>();
					component2.mS_ = mS_;
					component2.tS_ = tS_;
		...
		string text = tS_.GetText(1783) + ": " + uiObjects[0].transform.childCount;
		uiObjects[4].GetComponent<Text>().text = text;
	}

	public bool CheckGameData(gameScript script_)
	{
		if ((bool)script_ && !script_.pubAngebot && !script_.auftragsspiel && script_.IsMyIP(pS_) && script_.mainIP == script_.myID && CheckIP(script_))
		{
			return true;
		}
		return false;
	...
	public bool CheckIP(gameScript script_)
	{
		for (int i = 0; i < pS_.tf_ipFocus.Length; i++)
		{
			if (script_.myID == pS_.tf_ipFocus[i])
			{
				return false;
			}
		}
