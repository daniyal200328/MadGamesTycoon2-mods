...
public class Menu_Dev_KonsoleProSelect : MonoBehaviour
{
	...
	private bool Exists(GameObject parent_, int id_)
	{
		...
		{
			if (parent_.transform.GetChild(i).gameObject.activeSelf && parent_.transform.GetChild(i).GetComponent<Item_DevKonsolePro>().pS_.myID == id_)
			{
				return true;
			}
		}
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && CheckPlatformData(component))
			{
				...
				{
					Item_DevKonsolePro component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_DevKonsolePro>();
	...
	{
		if ((bool)script_ && script_.ownerID == mS_.myID && script_.isUnlocked && !script_.proVersion && script_.IsVerfuegbar())
		{
			return true;
		}
		return false;
	...
	public void DROPDOWN_Sort()
	{
		...
		for (int i = 0; i < childCount; i++)
		{
			...
			Item_DevKonsolePro component = gameObject.GetComponent<Item_DevKonsolePro>();
			switch (value)
			{
			case 0:
