...
public class Item_Genre_Forschung : MonoBehaviour
{
	...
	private void SetData()
	{
		...
		if ((float)genres_.genres_RES_POINTS[myID] == genres_.genres_RES_POINTS_LEFT[myID])
		{
			uiObjects[2].GetComponent<Text>().text = mS_.GetMoney(genres_.GetPrice(myID), showDollar: true);
		}
		else
		{
		...
		text = text.Replace("<NUM>", mS_.Round(genres_.genres_RES_POINTS_LEFT[myID], 2).ToString());
		uiObjects[3].GetComponent<Text>().text = text;
		float prozent = genres_.GetProzent(myID);
