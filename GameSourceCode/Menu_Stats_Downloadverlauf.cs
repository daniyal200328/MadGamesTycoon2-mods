...
public class Menu_Stats_Downloadverlauf : MonoBehaviour
{
	...
	private long InitBalken()
	{
		...
		float num2 = 400f;
		long num3 = 0L;
		for (int i = 0; i < mS_.downloadverlauf.Length; i++)
		{
			num += mS_.downloadverlauf[i];
			if (num3 < mS_.downloadverlauf[i])
			{
				num3 = mS_.downloadverlauf[i];
			}
		}
		float num4 = num2 / (float)num3;
		for (int j = 0; j < mS_.downloadverlauf.Length; j++)
		{
			uiBalken[j].GetComponent<Image>().fillAmount = (float)mS_.downloadverlauf[j] * num4 / num2;
			if (mS_.downloadverlauf[j] > 0)
			{
				uiBalken[j].transform.GetChild(0).GetComponent<Text>().text = mS_.GetMoney(mS_.downloadverlauf[j], showDollar: false);
				if (j < mS_.downloadverlauf.Length - 1)
				{
					long num5 = mS_.downloadverlauf[j] - mS_.downloadverlauf[j + 1];
					if (num5 > 0)
					{
