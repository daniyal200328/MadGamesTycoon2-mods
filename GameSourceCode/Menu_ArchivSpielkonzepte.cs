...
public class Menu_ArchivSpielkonzepte : MonoBehaviour
{
	...
	private void SetData(bool archiv_)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if ((bool)array[i])
			{
				...
				if ((bool)component && component.archiv_spielkonzept == archiv_ && (!isOn || (isOn && !component.typ_contractGame)) && CheckGameData(component) && !Exists(uiObjects[0], component.myID))
				{
					Item_ArchivSpielkonzept component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_ArchivSpielkonzept>();
					component2.mS_ = mS_;
					component2.tS_ = tS_;
	...
	{
		if ((bool)script_ && (script_.developerID == mS_.myID || script_.IsMyAuftragsspiel()) && !script_.typ_budget && !script_.typ_bundle && !script_.typ_bundleAddon && !script_.typ_goty && !script_.pubOffer)
		{
			return true;
		}
		return false;
