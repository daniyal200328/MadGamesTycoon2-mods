...
public class Menu_ShowKonsoleGames : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (sellsBonusForGames < 0f)
		{
			...
			uiObjects[4].GetComponent<Text>().text = text + " <color=#7D2A32>[" + mS_.Round(sellsBonusForGames, 2) + "%]</color>";
		}
		else
		{
			string text2 = tS_.GetText(297);
			text2 = text2.Replace("<NUM>", uiObjects[0].transform.childCount.ToString());
			uiObjects[4].GetComponent<Text>().text = text2 + " <color=#337D29>[+" + mS_.Round(sellsBonusForGames, 2) + "%]</color>";
		}
		MultiplayerUpdate();
	}
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if ((bool)array[i])
			{
				...
				{
					Item_MyGames_Review component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_MyGames_Review>();
					component2.mS_ = mS_;
					component2.tS_ = tS_;
	...
	{
		if ((bool)script_ && !script_.inDevelopment && !script_.auftragsspiel && !script_.typ_budget && !script_.typ_addon && !script_.typ_bundle && !script_.typ_bundleAddon && !script_.typ_goty && !script_.typ_addonStandalone)
		{
