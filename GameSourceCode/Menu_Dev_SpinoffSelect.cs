...
public class Menu_Dev_SpinoffSelect : MonoBehaviour
{
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && CheckGameData(component))
			{
				...
				{
					Item_DevGame_Spinoff component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_DevGame_Spinoff>();
	...
	{
		if ((bool)script_ && script_.ownerID == mS_.myID && !script_.inDevelopment && script_.mainIP == script_.myID && (!script_.pubOffer || script_.ownerID == mS_.myID) && !script_.typ_nachfolger && !script_.auftragsspiel && !script_.GetIpIsInArchiv())
		{
			return true;
		}
		return false;
