...
public class Menu_DevGame_CopyProtect : MonoBehaviour
{
	...
	private void FindScripts()
	{
		...
		if (!menuChangeCopyProtect_)
		{
			menuChangeCopyProtect_ = guiMain_.uiObjects[365].GetComponent<Menu_Dev_ChangeCopyProtect>();
		}
	}
	...
	private void Init()
	{
		...
		{
			if (devGame_.g_GameCopyProtect != -1)
			{
		...
		{
			if (menuChangeCopyProtect_.g_GameCopyProtect != -1)
			{
				uiObjects[4].GetComponent<Button>().interactable = true;
			}
			else
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && component.isUnlocked && component.inBesitz && (component.effekt > 0f || !isOn) && !Exists(uiObjects[0], component.myID))
			{
				...
				if (devGame_.g_GameCopyProtect == component.myID)
				{
	...
	public void BUTTON_CopyProtectEntfernen()
	{
		if (guiMain_.uiObjects[56].activeSelf)
		{
			devGame_.SetCopyProtect(-1);
		}
		if (guiMain_.uiObjects[365].activeSelf)
		{
			menuChangeCopyProtect_.SetCopyProtect(-1);
		}
		BUTTON_Close();
	}
}
