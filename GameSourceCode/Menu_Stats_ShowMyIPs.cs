...
public class Menu_Stats_ShowMyIPs : MonoBehaviour
{
	...
	private void ShowDaten()
	{
		...
		num /= 1000000f;
		uiObjects[19].GetComponent<Text>().text = mS_.Round(num, 2) + " " + tS_.GetText(1483);
		...
		float num2 = (float)gS_.merchGesamtGewinn / 1000000f;
		...
		if (num2 < 0f)
		{
			component = uiObjects[17].GetComponent<Text>();
			component.text = component.text + "<color=red>" + mS_.Round(num2, 2) + " " + tS_.GetText(1483) + "</color>\n";
		}
		else
		{
			component = uiObjects[17].GetComponent<Text>();
			component.text = component.text + "<color=green>" + mS_.Round(num2, 2) + " " + tS_.GetText(1483) + "</color>\n";
		}
		component = uiObjects[17].GetComponent<Text>();
		component.text = component.text + "<color=green>" + mS_.Round(gesamtUmsatz, 2) + " " + tS_.GetText(1483) + "</color>\n<color=red>" + mS_.Round(gesamtAusgaben, 2) + " " + tS_.GetText(1483) + "</color>\n\n";
		if (gesamtUmsatz - gesamtAusgaben + num2 >= 0f)
		{
			component = uiObjects[17].GetComponent<Text>();
			component.text = component.text + "<size=18><color=green>" + mS_.Round(gesamtUmsatz - gesamtAusgaben + num2, 2) + " " + tS_.GetText(1483) + "</color></size>";
		}
		else
		{
			component = uiObjects[17].GetComponent<Text>();
			component.text = component.text + "<size=18><color=red>" + mS_.Round(gesamtUmsatz - gesamtAusgaben + num2, 2) + " " + tS_.GetText(1483) + "</color></size>";
		}
	}
	...
	{
		if ((bool)script_ && script_.mainIP == gS_.myID)
		{
			return true;
		}
		return false;
	...
	public void BUTTON_Close()
	{
		if ((bool)gS_)
		{
			...
			if (mS_.multiplayer)
			{
				...
				if (mS_.mpCalls_.isClient && gS_.ownerID == mS_.myID)
				{
