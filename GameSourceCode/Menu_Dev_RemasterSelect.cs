...
public class Menu_Dev_RemasterSelect : MonoBehaviour
{
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && CheckGameData(component) && (value - 1 == component.maingenre || value == 0))
			{
				...
				{
					Item_DevGame_Remaster component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_DevGame_Remaster>();
	...
	{
		if ((bool)script_ && script_.ownerID == mS_.myID && !script_.inDevelopment && !script_.schublade && script_.gameTyp == 0 && script_.portID == -1 && !script_.auftragsspiel && (script_.typ_standard || script_.typ_nachfolger || script_.typ_spinoff) && !script_.remaster_created && !script_.isOnMarket && !script_.typ_budget && !script_.typ_goty && !script_.typ_remaster && !script_.typ_mmoaddon && !script_.typ_bundleAddon && !script_.typ_bundle && (!script_.pubOffer || script_.ownerID == mS_.myID) && !script_.GetIpIsInArchiv())
		{
			return true;
		}
		return false;
