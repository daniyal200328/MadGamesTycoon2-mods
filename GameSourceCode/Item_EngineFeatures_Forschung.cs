...
public class Item_EngineFeatures_Forschung : MonoBehaviour
{
	...
	private void SetData()
	{
		...
		uiObjects[7].GetComponent<Text>().text = eF_.engineFeatures_TECH[myID].ToString();
		if ((float)eF_.engineFeatures_RES_POINTS[myID] == eF_.engineFeatures_RES_POINTS_LEFT[myID])
		{
			uiObjects[2].GetComponent<Text>().text = mS_.GetMoney(eF_.GetPrice(myID), showDollar: true);
		}
		else
		{
		...
		text = text.Replace("<NUM>", mS_.Round(eF_.engineFeatures_RES_POINTS_LEFT[myID], 2).ToString());
		uiObjects[3].GetComponent<Text>().text = text;
		float prozent = eF_.GetProzent(myID);
