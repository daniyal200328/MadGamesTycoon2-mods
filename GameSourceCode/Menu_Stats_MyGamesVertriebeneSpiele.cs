...
public class Menu_Stats_MyGamesVertriebeneSpiele : MonoBehaviour
{
	...
	private bool Exists(GameObject parent_, int id_)
	{
		...
		{
			if (parent_.transform.GetChild(i).gameObject.activeSelf && parent_.transform.GetChild(i).GetComponent<Item_MyGames_VertriebeneSpiele>().game_.myID == id_)
			{
				return true;
			}
		}
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if ((bool)array[i])
			{
				...
				{
					Item_MyGames_VertriebeneSpiele component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_MyGames_VertriebeneSpiele>();
					component2.mS_ = mS_;
					component2.tS_ = tS_;
	...
	{
		if ((bool)script_ && script_.publisherID == mS_.myID && script_.pubOffer)
		{
			return true;
		}
		return false;
