...
public class Menu_QA_GameplayVerbessern : MonoBehaviour
{
	...
	private void UpdateGUI()
	{
		for (int i = 0; i < buttonAdds.Length; i++)
		{
			...
			{
				if (selectedGame.gameplayStudio[i])
				{
					uiObjects[i].GetComponent<Button>().interactable = false;
	...
	public void BUTTON_SelectGame()
	{
		...
		guiMain_.ActivateMenu(guiMain_.uiObjects[173]);
		guiMain_.uiObjects[173].GetComponent<Menu_QA_GameplayVerbessernSelectGame>().Init();
	}
	...
	public void BUTTON_Start()
	{
		...
		sfx_.PlaySound(3, force: true);
		mS_.Pay(num, 10);
		taskGameplayVerbessern taskGameplayVerbessern2 = guiMain_.AddTask_GameplayVerbessern();
		taskGameplayVerbessern2.Init(fromSavegame: false);
		taskGameplayVerbessern2.targetID = selectedGame.myID;
		for (int i = 0; i < buttonAdds.Length; i++)
		{
			if (buttonAdds[i])
			{
				taskGameplayVerbessern2.adds[i] = true;
			}
		}
		if (uiObjects[11].GetComponent<Toggle>().isOn)
		{
			taskGameplayVerbessern2.autoBugfix = true;
		}
		else
		{
			taskGameplayVerbessern2.autoBugfix = false;
		}
		taskGameplayVerbessern2.FindNewAdd();
		GameObject gameObject = GameObject.Find("Room_" + rS_.myID);
		if ((bool)gameObject)
		{
			gameObject.GetComponent<roomScript>().taskID = taskGameplayVerbessern2.myID;
		}
		guiMain_.CloseMenu();
		base.gameObject.SetActive(value: false);
	}
