...
public class Menu_Dev_AddonMMOSelectGame : MonoBehaviour
{
	...
	public void InitDropdowns()
	{
		...
		list.Add(tS_.GetText(1236));
		list.Add(tS_.GetText(956));
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && CheckGameData(component) && (value - 1 == component.maingenre || value == 0))
			{
				...
				{
					Item_DevGame_MMOAddon component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_DevGame_MMOAddon>();
	...
	{
		if ((bool)script_ && script_.ownerID == mS_.myID && script_.developerID == mS_.myID && !script_.inDevelopment && script_.isOnMarket && script_.gameTyp == 1 && !script_.pubOffer && !script_.handy && !script_.arcade && !script_.typ_goty && !script_.typ_budget && !script_.typ_bundle && !script_.typ_bundleAddon && (script_.typ_standard || script_.typ_nachfolger || script_.typ_spinoff) && !script_.GetIpIsInArchiv())
		{
			return true;
		}
		return false;
		...
		for (int i = 0; i < childCount; i++)
		{
			...
			if ((bool)gameObject)
			{
				...
				case 5:
					...
					break;
				case 6:
					gameObject.name = component.game_.amountMMOAddons.ToString();
					break;
				}
			}
		}
