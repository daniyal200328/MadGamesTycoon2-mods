...
public class Menu_Dev_AntiCheatAddon : MonoBehaviour
{
	...
	private void Init()
	{
		...
		{
			if (devAddon_.g_GameAntiCheat != -1)
			{
		...
		{
			if (devMMOAddon_.g_GameAntiCheat != -1)
			{
				uiObjects[4].GetComponent<Button>().interactable = true;
			}
			else
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && component.isUnlocked && component.inBesitz && (component.effekt > 0f || !isOn) && !Exists(uiObjects[0], component.myID))
			{
				...
				if (guiMain_.uiObjects[193].activeSelf && devAddon_.g_GameAntiCheat == component.myID)
				{
					gameObject.GetComponent<Button>().interactable = false;
				}
				if (guiMain_.uiObjects[247].activeSelf && devMMOAddon_.g_GameAntiCheat == component.myID)
				{
					gameObject.GetComponent<Button>().interactable = false;
				}
			}
	...
	public void BUTTON_AntiCheatEntfernen()
	{
		if (guiMain_.uiObjects[193].activeSelf)
		{
			devAddon_.SetAntiCheat(-1);
		}
		if (guiMain_.uiObjects[247].activeSelf)
		{
			devMMOAddon_.SetAntiCheat(-1);
		}
		BUTTON_Close();
	}
}
