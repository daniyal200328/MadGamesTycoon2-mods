...
public class Item_GameplayFeatures_Forschung : MonoBehaviour
{
	...
	private void SetData()
	{
		...
		if ((float)gF_.gameplayFeatures_RES_POINTS[myID] == gF_.gameplayFeatures_RES_POINTS_LEFT[myID])
		{
			uiObjects[2].GetComponent<Text>().text = mS_.GetMoney(gF_.GetPrice(myID), showDollar: true);
		}
		else
		{
		...
		text = text.Replace("<NUM>", mS_.Round(gF_.gameplayFeatures_RES_POINTS_LEFT[myID], 2).ToString());
		uiObjects[3].GetComponent<Text>().text = text;
		float prozent = gF_.GetProzent(myID);
		uiObjects[4].GetComponent<Image>().fillAmount = prozent * 0.01f;
		uiObjects[5].GetComponent<Text>().text = mS_.Round(prozent, 1) + "%";
		tooltip_.c = gF_.GetTooltip(myID, -1, -1);
	}
