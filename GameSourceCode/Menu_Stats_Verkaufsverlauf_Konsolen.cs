...
public class Menu_Stats_Verkaufsverlauf_Konsolen : MonoBehaviour
{
	...
	private long InitBalken()
	{
		...
		float num2 = 400f;
		long num3 = 0L;
		for (int i = 0; i < mS_.verkaufsverlaufKonsolen.Length; i++)
		{
			num += mS_.verkaufsverlaufKonsolen[i];
			if (num3 < mS_.verkaufsverlaufKonsolen[i])
			{
				num3 = mS_.verkaufsverlaufKonsolen[i];
			}
		}
		float num4 = num2 / (float)num3;
		for (int j = 0; j < mS_.verkaufsverlaufKonsolen.Length; j++)
		{
			uiBalken[j].GetComponent<Image>().fillAmount = (float)mS_.verkaufsverlaufKonsolen[j] * num4 / num2;
			if (mS_.verkaufsverlaufKonsolen[j] > 0)
			{
				uiBalken[j].transform.GetChild(0).GetComponent<Text>().text = mS_.GetMoney(mS_.verkaufsverlaufKonsolen[j], showDollar: false);
				if (j < mS_.verkaufsverlaufKonsolen.Length - 1)
				{
					long num5 = mS_.verkaufsverlaufKonsolen[j] - mS_.verkaufsverlaufKonsolen[j + 1];
					if (num5 > 0)
					{
