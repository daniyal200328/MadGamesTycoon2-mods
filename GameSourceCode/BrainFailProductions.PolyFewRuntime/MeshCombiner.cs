...
public static class MeshCombiner
{
	...
	{
		List<StaticRenderer> list = new List<StaticRenderer>(renderers.Length);
	...
	{
		List<SkinnedRenderer> list = new List<SkinnedRenderer>(renderers.Length);
	...
	public static StaticRenderer[] CombineStaticMeshes(Transform transform, int levelIndex, MeshRenderer[] renderers, bool autoName = true, string combinedBaseName = "")
	{
		...
		if (renderers.Length > 1)
		{
			...
			CombineMeshesUnity(transform, meshFilters);
		...
		{
			mesh = CombineMeshes(transform, renderers, out resultMaterials);
		}
		else if (unityCombinedMeshRenderers.Length != 1)
		{
			mesh = ((unityCombinedMeshRenderers.Length != 0) ? CombineMeshes(transform, unityCombinedMeshRenderers, out resultMaterials) : CombineMeshes(transform, renderers, out resultMaterials));
		}
		else
		{
			resultMaterials = unityCombinedMeshesMats.ToArray();
			mesh = unityCombinedMeshRenderers[0].GetComponent<MeshFilter>().sharedMesh;
		}
		if (unityCombinedMeshRenderers != null)
		{
			MeshRenderer[] array = unityCombinedMeshRenderers;
			for (int num = 0; num < array.Length; num++)
			{
				UnityEngine.Object.DestroyImmediate(array[num].gameObject);
			}
		}
		...
		string name = $"{arg}_combined_static";
	...
	public static SkinnedRenderer[] CombineSkinnedMeshes(Transform transform, int levelIndex, SkinnedMeshRenderer[] renderers, ref SkinnedMeshRenderer[] renderersActuallyCombined, bool autoName = true, string combinedBaseName = "")
	{
		...
		List<SkinnedRenderer> list = new List<SkinnedRenderer>(renderers.Length);
		IEnumerable<SkinnedMeshRenderer> enumerable = renderers.Where((SkinnedMeshRenderer renderer) => renderer.sharedMesh == null);
		SkinnedMeshRenderer[] array = (renderersActuallyCombined = renderers.Where((SkinnedMeshRenderer renderer) => renderer.sharedMesh != null).ToArray());
		foreach (SkinnedMeshRenderer item in enumerable)
		{
		...
		if (array.Length != 0)
		{
			...
			Mesh mesh = CombineMeshes(transform, array, out resultMaterials, out resultBones);
			string arg = (string.IsNullOrWhiteSpace(combinedBaseName) ? transform.name : combinedBaseName);
			string name = $"{arg}_combined_skinned";
			if (autoName)
			{
	...
	public static Mesh CombineMeshes(Transform rootTransform, MeshRenderer[] renderers, out Material[] resultMaterials, Dictionary<Transform, Transform> topLevelParents = null, Dictionary<string, BlendShapeFrame> blendShapes = null)
	{
		...
		for (int i = 0; i < renderers.Length; i++)
		{
			...
			array3[i] = Tuple.Create(rootTransform.localToWorldMatrix * transform.localToWorldMatrix, !flag2);
			array4[i] = meshRenderer.sharedMaterials;
		}
		return CombineMeshes(array, array2, array3, array4, out resultMaterials, blendShapes);
	}
	...
	public static Mesh CombineMeshes(Transform rootTransform, SkinnedMeshRenderer[] renderers, out Material[] resultMaterials, out Transform[] resultBones)
	{
		...
		Dictionary<string, BlendShapeFrame> dictionary = new Dictionary<string, BlendShapeFrame>();
		int num = 0;
		for (int i = 0; i < renderers.Length; i++)
		{
			...
			{
				array2[i] = rootTransform.worldToLocalMatrix * transform.localToWorldMatrix;
				array3[i] = Tuple.Create(rootTransform.localToWorldMatrix * transform.localToWorldMatrix, !flag);
			}
			else
			{
				array2[i] = transform.worldToLocalMatrix * transform.localToWorldMatrix;
				array3[i] = Tuple.Create(transform.worldToLocalMatrix * transform.localToWorldMatrix, !flag);
			}
			if (!flag)
			{
				Debug.LogWarning("The GameObject \"" + transform.name + "\" has non uniform scaling applied. This might cause the combined mesh normals to be incorrectly calculated resulting in slight variation in lighting.");
				...
				for (int k = 0; k < sharedMesh.blendShapeCount; k++)
				{
					for (int l = 0; l < sharedMesh.GetBlendShapeFrameCount(k); l++)
					{
						...
						if (!dictionary.ContainsKey(sharedMesh.GetBlendShapeName(k) + hashCode))
						{
							...
							dictionary.Add(sharedMesh.GetBlendShapeName(k) + hashCode, new BlendShapeFrame(sharedMesh.GetBlendShapeName(k) + hashCode, sharedMesh.GetBlendShapeFrameWeight(k, l), deltaVertices, deltaNormals, deltaTangents, num));
						}
					}
				}
			}
		...
		return CombineMeshes(array, array2, array3, array4, array5, out resultMaterials, out resultBones, dictionary);
	}
	...
	public static Mesh CombineMeshes(Mesh[] meshes, Matrix4x4[] transforms, Tuple<Matrix4x4, bool>[] normalsTransforms, Material[][] materials, Transform[][] bones, out Material[] resultMaterials, out Transform[] resultBones, Dictionary<string, BlendShapeFrame> blendShapes = null)
	{
		...
		for (int i = 0; i < meshes.Length; i++)
		{
			...
			for (int j = 0; j < array.Length; j++)
			{
				if (array[j] == null)
				{
					throw new ArgumentException($"The material at index {j} for mesh {mesh.name} is null.", "materials");
				}
			}
			if (bones == null)
			{
		...
		for (int l = 0; l < meshes.Length; l++)
		{
			...
			Tuple<Matrix4x4, bool> transform2 = normalsTransforms[l];
			...
			TransformNormals(normals, ref transform2);
			TransformTangents(tangents, ref transform2);
		...
		Mesh mesh3 = MeshUtils.CreateMesh(vertices2, indices, normals2, tangents2, colors2, boneWeights2, uvs, bindposes2, null);
		if (blendShapes != null && blendShapes.Count > 0)
		{
			foreach (BlendShapeFrame value2 in blendShapes.Values)
			{
				...
				for (int num7 = 0; num7 < value2.deltaVertices.Length; num7++)
				{
					array7.SetValue(value2.deltaVertices[num7], num7 + value2.vertexOffset);
					array8.SetValue(value2.deltaNormals[num7], num7 + value2.vertexOffset);
					array9.SetValue(value2.deltaTangents[num7], num7 + value2.vertexOffset);
				}
				mesh3.AddBlendShapeFrame(value2.shapeName, value2.frameWeight, array7, array8, array9);
			}
		}
	...
	private static void CombineMeshesUnity(Transform parentTransform, MeshFilter[] meshFilters)
	{
		...
		foreach (MeshFilter meshFilter2 in array)
		{
			...
			{
				throw new ArgumentException($"The materials for mesh {sharedMesh2.name} on GameObject {meshFilter2.transform.name} doesn't match the submesh count ({sharedMaterials.Length} != {sharedMesh2.subMeshCount}).", "materials");
			}
			for (int j = 0; j < sharedMaterials.Length; j++)
			{
				if (sharedMaterials[j] == null)
				{
					throw new ArgumentException($"The material at index {j} for mesh {sharedMesh2.name} on GameObject {meshFilter2.transform.name} is null.", "materials");
				}
			}
			for (int k = 0; k < subMeshCount; k++)
			{
				...
				Mesh mesh = UnityEngine.Object.Instantiate(sharedMesh2);
		...
		unityCombinedMeshRenderers = new MeshRenderer[dictionary.Count];
		unityCombinedMeshesMats = new Material[dictionary.Count];
		int num2 = 0;
		foreach (KeyValuePair<Material, List<CombineInstance>> item2 in dictionary)
		{
			GameObject gameObject = new GameObject(item2.Key.name);
			MeshRenderer meshRenderer = gameObject.AddComponent<MeshRenderer>();
			MeshFilter meshFilter3 = gameObject.AddComponent<MeshFilter>();
			meshRenderer.material = item2.Key;
			Mesh mesh2 = new Mesh();
			if (num > 65534)
			{
				mesh2.indexFormat = IndexFormat.UInt32;
			}
			mesh2.CombineMeshes(item2.Value.ToArray());
			meshFilter3.sharedMesh = mesh2;
			gameObject.transform.parent = parentTransform.parent;
			unityCombinedMeshesMats[num2] = item2.Key;
			unityCombinedMeshRenderers[num2] = meshRenderer;
			num2++;
		}
	}
}
