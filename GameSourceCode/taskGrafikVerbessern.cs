...
public class taskGrafikVerbessern : MonoBehaviour
{
	...
	private void FindScripts()
	{
		if (!main_)
		{
			...
			if (!menuGFX_)
			{
				menuGFX_ = guiMain_.uiObjects[174].GetComponent<Menu_GFX_GrafikVerbessern>();
			}
		}
	}
	...
	{
		return games_.gameAdds[aktuellerAdd + 6];
	}
	...
	public void FindNewAdd()
	{
		...
		if (aktuellerAdd != -1)
		{
			...
			points = num * menuGFX_.pointsInPercent[aktuellerAdd];
			pointsLeft = points;
		}
		else
		{
	...
	private void Complete()
	{
		...
		gS_.grafikStudio[aktuellerAdd] = true;
		gS_.costs_entwicklung += menuGFX_.GetCosts(aktuellerAdd, gS_);
		int roomID_ = FindMyRoomWithTask();
		string text = tS_.GetText(918);
	...
	public int GetRueckgeld()
	{
		...
		for (int i = 0; i < adds.Length; i++)
		{
			...
			{
				num += (float)menuGFX_.GetCosts(i, gS_);
			}
		}
		float num2 = menuGFX_.GetCosts(aktuellerAdd, gS_);
