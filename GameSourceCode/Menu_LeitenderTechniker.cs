...
public class Menu_LeitenderTechniker : MonoBehaviour
{
	...
	private void FindScripts()
	{
		...
		{
			menuEntwicklungsbericht_ = guiMain_.uiObjects[325].GetComponent<Menu_Dev_KonsoleEntwicklungsbericht>();
		}
	}
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((uiObjects[7].GetComponent<InputField>().text.Length <= 0 || myName.Contains(searchStringA)) && !Exists(uiObjects[0], component.myID))
			{
				...
				if (menuDevKonsole_.gameObject.activeSelf && menuDevKonsole_.leitenderTechniker == component)
				{
					gameObject.GetComponent<Button>().interactable = false;
				}
				if (menuEntwicklungsbericht_.gameObject.activeSelf && menuEntwicklungsbericht_.GetLeitenderTechniker() == component)
				{
					gameObject.GetComponent<Button>().interactable = false;
				}
			}
	...
	public void SetAuto()
	{
		...
		if (uiObjects[8].GetComponent<Toggle>().isOn)
		{
			...
			if (menuDevKonsole_.gameObject.activeSelf)
			{
				menuDevKonsole_.SetLeitenderTechniker(null, manuellSelectet: false);
			}
			if (menuEntwicklungsbericht_.gameObject.activeSelf)
			{
				menuEntwicklungsbericht_.SetLeitenderTechniker(null, manuellSelectet: false);
			}
		}
		else
		{
			if (menuDevKonsole_.gameObject.activeSelf && (bool)menuDevKonsole_.leitenderTechniker)
			{
				rS_.leitenderTechniker = menuDevKonsole_.leitenderTechniker.myID;
			}
			if (menuEntwicklungsbericht_.gameObject.activeSelf && (bool)menuEntwicklungsbericht_.GetLeitenderTechniker())
			{
				rS_.leitenderTechniker = menuEntwicklungsbericht_.GetLeitenderTechniker().myID;
			}
		}
	}
