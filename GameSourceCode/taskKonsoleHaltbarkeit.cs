...
public class taskKonsoleHaltbarkeit : MonoBehaviour
{
	public int myID = -1;
	...
	public Menu_Dev_KonsoleHaltbarkeit menu_;
	...
	private void FindScripts()
	{
		if (!main_)
		{
			...
			if (!menu_)
			{
				menu_ = guiMain_.uiObjects[452].GetComponent<Menu_Dev_KonsoleHaltbarkeit>();
			}
		}
	}
	...
	private void Complete()
	{
		...
		pS_.haltbarkeitDone[aktuellerAdd] = true;
		pS_.costs_haltbarkeit += (int)menu_.GetCosts(aktuellerAdd, pS_);
		pS_.AddHaltbarkeitFloat(menu_.haltbarkeitProKampagne[aktuellerAdd]);
		int roomID_ = FindMyRoomWithTask();
		string text = tS_.GetText(2378);
