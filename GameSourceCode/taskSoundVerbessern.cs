...
public class taskSoundVerbessern : MonoBehaviour
{
	...
	private void FindScripts()
	{
		if (!main_)
		{
			...
			if (!menuSFX_)
			{
				menuSFX_ = guiMain_.uiObjects[176].GetComponent<Menu_SFX_SoundVerbessern>();
			}
		}
	}
	...
	{
		return games_.gameAdds[aktuellerAdd + 12];
	}
	...
	public void FindNewAdd()
	{
		...
		if (aktuellerAdd != -1)
		{
			...
			points = num * menuSFX_.pointsInPercent[aktuellerAdd];
			pointsLeft = points;
		}
		else
		{
	...
	private void Complete()
	{
		...
		gS_.soundStudio[aktuellerAdd] = true;
		gS_.costs_entwicklung += menuSFX_.GetCosts(aktuellerAdd, gS_);
		int roomID_ = FindMyRoomWithTask();
		string text = tS_.GetText(921);
	...
	public int GetRueckgeld()
	{
		...
		for (int i = 0; i < adds.Length; i++)
		{
			...
			{
				num += (float)menuSFX_.GetCosts(i, gS_);
			}
		}
		float num2 = menuSFX_.GetCosts(aktuellerAdd, gS_);
