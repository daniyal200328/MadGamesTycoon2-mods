...
public class Menu_GFX_GrafikVerbessern : MonoBehaviour
{
	...
	private void UpdateGUI()
	{
		for (int i = 0; i < buttonAdds.Length; i++)
		{
			...
			{
				if (selectedGame.grafikStudio[i])
				{
					uiObjects[i].GetComponent<Button>().interactable = false;
		...
		if ((bool)selectedGame && !selectedGame.gameGameplayFeatures[40])
		{
			...
			text = text.Replace("<NAME>", gF_.GetName(40));
	...
	public void BUTTON_SelectGame()
	{
		...
		guiMain_.ActivateMenu(guiMain_.uiObjects[175]);
		guiMain_.uiObjects[175].GetComponent<Menu_GFX_GrafikVerbessernSelectGame>().Init();
	}
	...
	public void BUTTON_Start()
	{
		...
		sfx_.PlaySound(3, force: true);
		mS_.Pay(num, 10);
		taskGrafikVerbessern taskGrafikVerbessern2 = guiMain_.AddTask_GrafikVerbessern();
		taskGrafikVerbessern2.Init(fromSavegame: false);
		taskGrafikVerbessern2.targetID = selectedGame.myID;
		for (int i = 0; i < buttonAdds.Length; i++)
		{
			if (buttonAdds[i])
			{
				taskGrafikVerbessern2.adds[i] = true;
			}
		}
		taskGrafikVerbessern2.FindNewAdd();
		GameObject gameObject = GameObject.Find("Room_" + rS_.myID);
		if ((bool)gameObject)
		{
			gameObject.GetComponent<roomScript>().taskID = taskGrafikVerbessern2.myID;
		}
		guiMain_.CloseMenu();
		base.gameObject.SetActive(value: false);
	}
