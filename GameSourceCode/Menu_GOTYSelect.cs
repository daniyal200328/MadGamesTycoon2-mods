...
public class Menu_GOTYSelect : MonoBehaviour
{
	...
	private void SetData()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if ((bool)component && (!isOn || (isOn && !component.GetIpIsInArchiv())) && CheckGameData(component))
			{
				...
				{
					Item_GotySelect component2 = Object.Instantiate(uiPrefabs[0], new Vector3(0f, 0f, 0f), Quaternion.identity, uiObjects[0].transform).GetComponent<Item_GotySelect>();
	...
	{
		if ((bool)script_ && script_.ownerID == mS_.myID && !script_.inDevelopment && !script_.isOnMarket && script_.gameTyp == 0 && !script_.schublade && script_.goty && !script_.goty_created && !script_.pubOffer && (script_.typ_standard || script_.typ_nachfolger || script_.typ_remaster || script_.typ_spinoff) && !script_.handy && !script_.arcade && !script_.freeware)
		{
			return true;
		}
		return false;
